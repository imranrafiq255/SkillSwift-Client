...
const ServiceHistoryPage = ({ services }) => {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-6">
        ...
        <div className="hidden md:grid grid-cols-1 gap-6">
          {services
            .filter(service => {
              if (activeTab === 'inProgress') return service.status === 'inProgress';
              if (activeTab === 'completed') return service.status === 'completed';
              if (activeTab === 'toReview') return service.status === 'completed';
            ...
            .map(service => (
              <div key={service.id} className="bg-white p-4 rounded-lg shadow-md flex items-center justify-between">
                ...
                >
                  {activeTab === 'inProgress' ? 'Cancel' : activeTab === 'toReview' ? 'Review' : 'Book Again'}
                </button>
              </div>
            ))}
        </div>
            ...
            <div className="flex space-x-1 mb-4">
              {[...Array(5)].map((_, index) => (
                <svg
                  ...
                  className={`w-8 h-8 cursor-pointer ${rating > index ? 'text-yellow-500' : 'text-gray-300'}`}
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
